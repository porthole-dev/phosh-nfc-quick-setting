...
#
# The app has no published tarball yet, so pmaports/temp/phosh-nfc-quick-setting builds from one
...
aport=$here/../../pmaports/temp/phosh-nfc-quick-setting
...
tarball=phosh-nfc-quick-setting-$version.tar.gz

tar -czf "$aport/$tarball" \
    --exclude=_build --exclude=.git --exclude="*.tar.gz" \
    --transform "s,^\.,phosh-nfc-quick-setting-$version," \
    -C "$here" .
